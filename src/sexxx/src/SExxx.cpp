...
INT32 SExxx::connect(std::string hostPC, int portPC)
{
 /* UDP */
 /* server  */
...
#if 0
void SExxx::connect(std::string host, int port)
{
  /* tcp */
  if (!connected_)
  {
...
bool SExxx::isConnected()
{
  return connected_;
}


void SExxx::InvertUint8(unsigned char *dBuf,unsigned char *srcBuf)
{
    int i;
    unsigned char tmp[4];
    tmp[0] = 0;
...
void SExxx::InvertUint16(unsigned short *dBuf,unsigned short *srcBuf)
{
    int i;
...
unsigned short  SExxx::CRC16(unsigned char *puchMsg, unsigned int usDataLen)
{
...
void SExxx::StartMeasureTransmission()
{
  INT32  ulSendLength=0;
  BITS32 ulMsgId = NULL_MSGID;
...
INT32 SExxx::PackUserParaSyncReq(USER_PARA_SYNC_REQ_T *vpstUserParaSyncReq, BITS8 *vpucBuff)
{
    ...
    {
        logWarn("PackUserParaSyncReq:  Pointer address is invalid\n");
...
INT32 SExxx::PackStartMeasureTransmissionReq(MEAS_DATA_HEAD_REQ_T *vpstMeasParaReq, BITS8 *vpucBuff)
{
    BITS8 *pucSendNum = vpucBuff ;
    BITS8  ucIndex=0;
    BITS8  ucIndex1=0;
...
INT32 SExxx::UnpackUserParaSyncRsp(BITS8 *vpucMsg, USER_PARA_SYNC_RSP_T *vpstUserParaSyncRsp, INT32 lLength)
{
    BITS32  ulIndex=0;
    INT32   lTmpLength=0;
    ...
    {
        logWarn("UnpackUserParaSyncRsp:  Pointer address is invalid\n");
        return BUSPRO_ERROR_POINTER;
    }
...
INT32 SExxx::UnpackMeasData(BITS8 *vpucMsg, MEAS_DATA_RSP_T *vpstMeasDataRsp)
{
    BITS8 *pucRecvNum = vpucMsg ;
    BITS32  ulIndex=0;
    ...
    {
        logWarn("UnpackMeasData:  Pointer address is invalid\n");
        return BUSPRO_ERROR_POINTER;
    }
...
void SExxx::send_data (void* vpSrc, BITS16 usCnt)
{
	/* tcp */
...
* 函数功能: 读取数据
***********************************************************/
INT32 SExxx::read_data (void* vpSrc, BITS16 usCnt)
{
       /* tcp */
